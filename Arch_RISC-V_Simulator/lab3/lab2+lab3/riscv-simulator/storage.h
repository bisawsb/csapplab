...
typedef struct StorageStats_ {
    ...
    int miss_num = 0;
    int access_time = 0; // In nanoseconds
    int replace_num = 0; // Evict old lines
    int fetch_num = 0; // Fetch lower layer
    int prefetch_num = 0; // Prefetch
} StorageStats;
...
public:
  ...
  // [out] time: total access time
    virtual void HandleRequest(uint32_t addr, int bytes, int read,
                               char* storage, int &hit, int &time) = 0;
