...
int main(int argc, char *argv[])
{
    ...
    // build machine
	if(pipeline){
		machine_p = new ns2::Machine(predType);
		machine_p->singleStep = singleStep;
		machine_p->cfg.LoadConfig(cfgFileName.c_str());
	}
	else
		machine = new ns1::Machine(singleStep);
	...
	for (int i = 0; i < n_seg; ++i)
    {
        ...
        for (uint64_t offset = 0; addr < m_end; addr++, offset++)
            if(pipeline)	machine_p->WriteMem(addr, 1, data[offset]);
			else	machine->WriteMem(addr, 1, data[offset]);
    }

    if(pipeline)	machine_p->WriteReg(P_PCReg, elf.get_entry());
	else	machine->WriteReg(PCReg, elf.get_entry());
