...
void learn_smac(u8 inport,u8 *src_mac)
{
	...
	//TODO User add code
	for(j = 0;j < NM08_NEIGH_MAX; j++)
		if(nm08_table->mac[j].valid == 1 && !ether_addr_equal(nm08_table->mac[j].addr, src_mac)){
			update_mac_time(inport, j);
			flag = 1;
			xprintf("learn_smac->update MAC,port:%d,index:%d\n",inport,j);
			break;
		}
	...
	if(!flag)
	{
		for(i = 0 ;i < NM08_NEIGH_MAX; i++)
			if(nm08_table->mac[i].valid == 0)
				break;
		j = i;
		nm08_table->mac[j].valid = 1;
		/*添加地址*/
		for(i = 0; i < MAC_LEN; i++)
			nm08_table->mac[j].addr[i] = *(src_mac+i);
		update_mac_time(inport, j);
...
int find_dmac(u8 inport,u8 *dst_mac)
{
	...
	//TODO User add code
	for(i = 0 ;i < NM08_NEIGH_MAX; i++)
		if(nm08_table->mac[i].valid == 1 && !ether_addr_equal(nm08_table->mac[i].addr, dst_mac))
		{
			ret = (nm08_table->mac[i].port == inport ? -2 : nm08_table->mac[i].port); /*返回-2表示目的端口与传入端口一致*/
			break;
		}
...
void pkt_send_flood(struct fast_packet *pkt,int pkt_len)
{
	...
	for(i = 0; i < NM08_PORT_CNT; i++)
	{
		if(i == inport)
			continue;
		struct fast_packet pkt_ = *pkt;
		pkt_.um.outport = i;
		pkt_send_normal(&pkt_, pkt_len);
	}
}
...
void *nm08_mac_aging(void *argv)
{
	...
	while(1)
	{
		...
		//TODO User add code
		for(i = 0; i < NM08_NEIGH_MAX; i++)
			if(nm08_table->mac[i].valid == 1 && tv_now.tv_sec > nm08_table->mac[i].tv.tv_sec)
			{
