...
void pkt_send_flood(struct fast_packet *pkt,int pkt_len)
{
	...
	for(;i< FAST_ROUTE_PORT_CNT;i++)/*除输入端口外，其他都发送一份*/
	{
		//TODO User add code
	}
}
...
void arp_do_response(struct fast_packet *pkt)
{
	...
	ar->ar_op = htons(2);/*ARP 应答*/
	exchange_value(eth->dmac,eth->smac,6);
	exchange_value(ar->ar_tha,ar->ar_sha,6);
	exchange_value((u8 *)&ar->ar_tip,(u8 *)&ar->ar_sip,4);
	memcpy(ar->ar_sha,ports[pkt->um.inport].mac,6);/*修改ARP协议中的源MAC地址*/
	memcpy(eth->smac,ports[pkt->um.inport].mac,6);/*修改以太网帧的源MAC地址*/

	pkt->um.outport = pkt->um.inport;/*ARP回应报文原端口返回*/
	pkt_send_normal(pkt,pkt->um.len);
}
...
int find_mac_by_ip4(u32 ip,int port,int *idx)
{
	...
	{		
		if(ports[port].neigh[i].ip4 == ip)/*在邻接表中找到了对应IP地址*/
		{
			//TODO User add code
		}	
	}
...
/*处理队列报文线程，该部分数据不能直接替换二层数据转发是因为：
 * 该报文的网关（下一跳，或直接目的IP地址）的MAC地址在邻接表中不存在（或老化）
 * 故需要根据IP地址去重新学习对应的MAC地址后才能封装新的二层数据转发
 */
	...
	while(poll)
	{
		...
		for(i=0;i<FAST_NEIGH_MAX;i++)
		{
			...
			{		
				...
				{
					while((send_buf=rt_dequeue(&ports[portidx].list[i])))/*只当前IP有MAC，就取其队列中报文出来发送*/
					{		
						/*构造ICMP不可达信息返回主机端*/
			...
			{
				if(ports[portidx].neigh[j].time != 0 && ports[portidx].list[j].qlen > 0)/*说明该表项上的IP地址学习到了MAC地址*/
				{
					/*如果队列过大，此处是否可以做个限额发送？？？？*/
					while((send_buf=rt_dequeue(&ports[portidx].list[j])))/*只当前IP有MAC，就取其队列中报文出来发送*/
					{			
						sendflag = 1;
						pack_l2_send(&send_buf->pkt,portidx,j);/*第J项邻接表学到MAC了，可以将挂在此队列的报文全部输出了*/
						free(send_buf);/*由于该报文是重新申请内容，拷贝报文内容后存入队列的，在发送完成后应该释放其内存*/
					}
				}
					
			}
		}
		if(arpflag + sendflag == 0)/*如果此轮没有报文处理，说明所有转发报文均能正常查询得到MAC进行转发*/
		{
...
int handle_arp(struct fast_packet *pkt)
{
	...
	if(ar->ar_tip == ports[pkt->um.inport].ip4)/*ARP请求当前接口的IPv4地址*/
	{
		...
		{
			xprintf("ARP->Request," NIPV4FMT "[" NMACFMT "]--> " NIPV4FMT "[???]\n",NIPV4ADDR(ar->ar_sip),NMACADDR(ar->ar_sha),NIPV4ADDR(ar->ar_tip));
			arp_do_response(pkt);
			return 0;
		}
		else/*ARP Response*/
		{			
			xprintf("ARP->Response," NIPV4FMT "[" NMACFMT "]--> " NIPV4FMT "[" NMACFMT "]\n",NIPV4ADDR(ar->ar_sip),NMACADDR(ar->ar_sha),NIPV4ADDR(ar->ar_tip),NMACADDR(ar->ar_tha));
			learn_smac(pkt);
			pkt->um.dstmid = 128;/*送协议栈*/
			return fast_ua_send(pkt,pkt->um.len);/*让协议栈学习到邻接信息（ICMP、控制协议等等依赖协议栈工作）*/
		}		
	}/*请求其他IP不处理，路由的广播阻断功能*/
...
int output_dataplane_pkt(struct fast_packet *pkt,int tlbidx)
{
	...
	if(gw == 0)/*网关（下一跳）为零，说明直接转发，网关即下目的IP地址*/
		gw = ip4->dst;
	if(outport == -1)/*查表不命中会返回默认路由，若无默认路由，则可能导致输出端口为-1*/
	{
		xprintf("ICMP Report and Drop!\n");
		return;
	}
	...
	if((macidx=find_mac_by_ip4(gw,outport,&ipidx))>-1)/*查询到了目的MAC地址，可以直接封装二层数据后进行转发*/
	{
		pthread_mutex_unlock(&ports[outport].neigh_lock);/*释放锁后再返回*/
		xprintf("Data Plane->port[%d],NEIGH[%d],DMAC->" NMACFMT "\n",outport,macidx,NMACADDR(ports[outport].neigh[macidx]));
		return pack_l2_send(pkt,outport,macidx);/*封装二层数据后发送*/		
	}
	else if(ipidx != -1)/*查不到此报文网关（下一跳）的MAC地址，需要构造ARP请求获取后再发送，现将其提交到请求处理队列*/
	{
		/* ipidx != -1 说明为此网关地址搜索到了一个可以存储在邻接表中的位置项，用来学习该IP的MAC地址*/
		...
		memcpy(&buf->pkt,pkt,pkt->um.len);/*一定要将此报文复制后进队列，因为当前pkt是UA的回调函数中的内存，此次回调结束后会重新用来接收新报文*/
		buf->gw = gw;/*保存该报文的网关地址，在APR请求MAC时要用*/
		rt_queue_tail(&ports[outport].list[ipidx],buf);/*将报文挂到该输出端口，并且是该端口邻接表中存储该网关地址对应的报文队列上*/
		queue_tail = 1;
	}	
	pthread_mutex_unlock(&ports[outport].neigh_lock);/*释放锁*/
	if(ports[outport].poll == 0 && queue_tail == 1)/*只要有入队操作，就要标记该端口轮询标志（当然还有此标志已经为0，说明线程停止处理数据了），启动队列处理线程工作*/
	{
...
{
	if(is_host_ether_addr(pkt->data,pkt->um.inport))/*目的MAC为本接口的MAC地址，说明是主机报文*/
	{
		xprintf("Recv Unicast Host Packet,Dispatch Packet!\n");
		return dispatch_pkt(pkt);
	}
	else if(is_multicast_ether_addr(pkt->data))/*路由器是否支持组播功能？*/
	{
...
{	
	...
	fast_reg_wr(FAST_ACTION_REG_ADDR|FAST_DEFAULT_RULE_ADDR,ACTION_SET_MID<<28|FAST_UA_DFT_L3ROUTE);	
	/*启动线程接收分派给UA进程的报文*/
	fast_ua_recv();
	while(1){sleep(9999);}
	return 0;
